...
#    dataset = dataset.map(map_fn, with_indices=True, remove_columns=dataset.column_names, num_proc=1)
#    return dataset

# 替换原 build_livecodebench_dataset 为：
def build_livecodebench_dataset(out_dir):
    """Stream LiveCodeBench to parquet to avoid OOM."""
    import base64, json, pickle, zlib

    def compress_testcases(example):
        public = json.loads(example["public_test_cases"])
        try:
            private = json.loads(example["private_test_cases"])
        except Exception:
            private = json.loads(pickle.loads(zlib.decompress(base64.b64decode(example["private_test_cases"].encode("utf-8")))))
        full = public + private
        test_cases = {
    ...
    dataset = (ex for ex in dataset if "2024-08-00T00:00:00" <= ex["contest_date"] < "2025-02-00T00:00:00")
        ...
        rows.append(
            {
                ...
                "ability": "Code",
                "reward_model": {"style": "rule", "ground_truth": compress_testcases(ex)},
                "extra_info": {
                    "contest_date": ex["contest_date"],
                    "contest_id": ex["contest_id"],
    ...
    if writer:
        writer.close()
    print(f"done, saved to {out_path}, total rows {count}")
...
if __name__ == "__main__":
    ...
    for task in args.tasks:
        if task == "livecodebench":
            build_livecodebench_dataset(args.local_dir)
        else:
