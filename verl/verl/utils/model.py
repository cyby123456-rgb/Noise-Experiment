...
def sample_quantile_fractions(batch: int, seq_len: int, num_quantiles: int, device, dtype, mode: str = "iqn") -> torch.Tensor:
    if mode == "iqn":
        return torch.rand(batch, seq_len, num_quantiles, device=device, dtype=dtype)
    # fixed mid-point fractions
    base = (torch.arange(num_quantiles, device=device, dtype=dtype) + 0.5) / num_quantiles
    return base.view(1, 1, num_quantiles).expand(batch, seq_len, num_quantiles)


class IQNValueHead(nn.Module):
    def __init__(self, hidden_size: int, num_cos_embeddings: int = 64, num_quantile_chunks: int = 2):
        super().__init__()
        self.num_cos_embeddings = num_cos_embeddings
        self.num_quantile_chunks = max(1, int(num_quantile_chunks))
        self.quantile_proj = nn.Linear(num_cos_embeddings, hidden_size)
        self.value_head = nn.Linear(hidden_size, 1)

    def forward(self, hidden_states: torch.Tensor, taus: torch.Tensor) -> torch.Tensor:
        device = hidden_states.device
        dtype = hidden_states.dtype
        cos_freq = torch.arange(1, self.num_cos_embeddings + 1, device=device, dtype=dtype)
        cos_emb = torch.cos(taus.unsqueeze(-1) * cos_freq * torch.pi)  # (b, t, k, num_cos)
        tau_emb = torch.relu(self.quantile_proj(cos_emb))  # (b, t, k, hidden)
        mixed = hidden_states.unsqueeze(2) * tau_emb  # broadcast hidden over quantiles
        return self.value_head(mixed).squeeze(-1)  # (b, t, k)
        # Compute IQN head in two chunks along quantile dim to reduce peak memory.
        #k = taus.size(2)
        #num_chunks = min(self.num_quantile_chunks, k)
        #outputs = []
        #hidden_states_expanded = hidden_states.unsqueeze(2)
        #def _forward_chunk(hs_expanded: torch.Tensor, taus_chunk: torch.Tensor) -> torch.Tensor:
        #    taus_chunk = taus_chunk.contiguous()
        #    cos_emb = torch.cos(taus_chunk.unsqueeze(-1) * cos_freq * torch.pi)  # (b, t, k_chunk, num_cos)
        #    tau_emb = torch.relu(self.quantile_proj(cos_emb))  # (b, t, k_chunk, hidden)
        #    mixed = hs_expanded * tau_emb  # broadcast hidden over quantiles
        #    return self.value_head(mixed).squeeze(-1)  # (b, t, k_chunk)

        #for taus_chunk in torch.chunk(taus, chunks=num_chunks, dim=2):
        #    taus_chunk = taus_chunk.contiguous()
        #    chunk_out = _forward_chunk(hidden_states_expanded, taus_chunk)
        #    outputs.append(chunk_out)
        #return torch.cat(outputs, dim=2)  # (b, t, k)

class MultiHeadIQNValueHead(nn.Module):
    def __init__(self, hidden_size: int, num_heads: int, num_cos_embeddings: int = 64):
        super().__init__()
        self.num_heads = num_heads
        self.heads = nn.ModuleList(
            [IQNValueHead(hidden_size=hidden_size, num_cos_embeddings=num_cos_embeddings) for _ in range(num_heads)]
        )

    def forward(self, hidden_states: torch.Tensor, taus: torch.Tensor) -> torch.Tensor:
        head_outputs = [head(hidden_states, taus) for head in self.heads]
        return torch.stack(head_outputs, dim=2)  # (b, t, h, k)   

class QRValueHead(nn.Module):
    def __init__(self, hidden_size: int, num_quantiles: int):
        super().__init__()
        self.value_head = nn.Linear(hidden_size, num_quantiles)
...
class C51ValueHead(nn.Module):
    def __init__(self, hidden_size: int, num_atoms: int):
        super().__init__()
        self.value_head = nn.Linear(hidden_size, num_atoms)
