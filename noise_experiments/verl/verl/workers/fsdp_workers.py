...
from verl.utils.model import C51ValueHead, IQNValueHead, MultiHeadIQNValueHead, QRValueHead, compute_position_id_with_mask
...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        with init_context(), warnings.catch_warnings():
            ...
            # add legacy random hidden-state noise only when shared training noise is not enabled
            ...
            if train_hidden_noise_cfg is not None:
                try:
                    ...
                    train_hidden_noise_std = 0.0
            if role =="actor" and train_hidden_noise_std <= 0:
    ...
    def init_model(self):
        ...
        if self._is_rollout:
            ...
            with open_dict(self.config.actor):
                self.config.actor.use_remove_padding = use_remove_padding
            self.rollout_policy_analyzer = DataParallelPPOActor(config=self.config.actor, actor_module=self.actor_module_fsdp, actor_optimizer=None)
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            # perform training
            with self._apply_shared_hidden_noise_if_needed(self.actor_module, data.meta_info.get("shared_hidden_noise"), apply_phase="train"):
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        shared_hidden_noise_cfg = prompts.meta_info.get("shared_hidden_noise")
        ...
        with self.rollout_sharding_manager:
            ...
            if is_validate and validation_noise_cfg:
                ...
                with self._apply_shared_hidden_noise_if_needed(rollout_model, shared_hidden_noise_cfg, apply_phase="eval"):
                    if self.config.rollout.name == "sglang_async":
                        from verl.workers.rollout.sglang_rollout import AsyncSGLangRollout
                        if isinstance(self.rollout, AsyncSGLangRollout) and hasattr(self.rollout, "_tool_schemas") and len(self.rollout._tool_schemas) > 0:
                            output = self.rollout.generate_sequences_with_tools(prompts=prompts)
    ...
    def _apply_validation_noise_if_needed(self, noise_cfg, is_validate):
        ...
        model = self._get_rollout_model_for_hidden_noise()
        with self._apply_validation_noise_to_model_if_needed(model, noise_cfg, is_validate):
            yield

    @contextmanager
    def _apply_validation_noise_to_model_if_needed(self, model, noise_cfg, is_validate):
        ...
        layer_seed = noise_cfg.get("layer_seed", None)

        if isinstance(layer_idx, str):
            layer_idx = None if layer_idx.lower() == "null" else layer_idx
        if hasattr(layer_idx, "__iter__") and not isinstance(layer_idx, (str, bytes)):
            try:
                layer_idx = [int(idx) for idx in layer_idx]
            except (TypeError, ValueError):
                layer_idx = None
        ...
        std_eff = self._resolve_hidden_noise_std(noise_cfg)
    ...
    def _get_rollout_model_for_hidden_noise(self):
        if self.config.rollout.name != "vllm":
            return None
        model_runner = getattr(self.rollout_sharding_manager, "model_runner", None)
        return getattr(model_runner, "model", None) if model_runner is not None else None

    def _resolve_hidden_noise_schedule_value(self, noise_cfg):
        if not noise_cfg:
            return None
        schedule_cfg = noise_cfg.get("schedule", None)
        if not schedule_cfg:
            return None
        ...
        if schedule_type in (None, "", "null", "none"):
            return None
        if schedule_type != "piecewise_linear":
            return None

        raw_points = schedule_cfg.get("points", []) or []
        points = []
        for point in raw_points:
            if not isinstance(point, dict):
                continue
            try:
                step = float(point.get("step"))
        ...
        try:
            step_val = float(noise_cfg.get("global_step", 0))
        except (TypeError, ValueError):
            step_val = 0.0

        if step_val <= points[0][0]:
            return max(0.0, points[0][1])
        if step_val >= points[-1][0]:
            return max(0.0, points[-1][1])

        for (left_step, left_value), (right_step, right_value) in zip(points[:-1], points[1:]):
            if step_val <= right_step:
                if right_step <= left_step:
                    return max(0.0, right_value)
                ratio = (step_val - left_step) / (right_step - left_step)
                value = left_value + ratio * (right_value - left_value)
                return max(0.0, value)

        return max(0.0, points[-1][1])

    def _resolve_hidden_noise_std(self, noise_cfg):
        if not noise_cfg:
            return 0.0
        scheduled_std = self._resolve_hidden_noise_schedule_value(noise_cfg)
        if scheduled_std is not None:
            try:
                adaptive_scale = float(noise_cfg.get("adaptive_scale", 1.0))
            except (TypeError, ValueError):
                adaptive_scale = 1.0
            return max(0.0, scheduled_std * adaptive_scale)
        try:
        ...
        if decay_steps not in (None, "null"):
            ...
            if decay_steps_val is not None and decay_steps_val > 0:
                ratio = max(0.0, 1.0 - step_val / decay_steps_val)
                std = max(min_std, std * ratio)
        return std

    def _derive_hidden_noise_seed(self, noise_cfg):
        if not noise_cfg:
            return None
        ...
        try:
            ...
            payload = str(seed).encode("utf-8", errors="backslashreplace")
            digest = hashlib.sha256(payload).digest()
            return int.from_bytes(digest[:8], byteorder="little", signed=False) % (2**63 - 1)

    @contextmanager
    def _apply_shared_hidden_noise_if_needed(self, model, noise_cfg, apply_phase: str):
        if model is None or not noise_cfg:
            yield
            return
        std = self._resolve_hidden_noise_std(noise_cfg)
        if std <= 0:
            yield
            return
        layer_idx = noise_cfg.get("layer_idx", None)
        if isinstance(layer_idx, str):
            layer_idx = None if layer_idx.lower() == "null" else int(layer_idx)
        elif hasattr(layer_idx, "__iter__") and not isinstance(layer_idx, (str, bytes)):
            layer_idx = [int(x) for x in layer_idx]
        elif layer_idx is not None:
            layer_idx = int(layer_idx)
        all_layers = bool(noise_cfg.get("all_layers", False))
        seed = self._derive_hidden_noise_seed(noise_cfg)
        handles = register_hidden_state_noise(
            model=model,
            std=std,
            layer_idx=layer_idx,
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            with self._apply_shared_hidden_noise_if_needed(self.actor_module, data.meta_info.get("shared_hidden_noise"), apply_phase="eval"):
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def analyze_generation_policy(self, data: DataProto):
        assert self._is_rollout
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        ...
        data = data.to(torch.cuda.current_device())
        data.meta_info["micro_batch_size"] = self.config.rollout.log_prob_micro_batch_size_per_gpu
        data.meta_info["max_token_len"] = self.config.rollout.get("log_prob_max_token_len_per_gpu", self.config.rollout.max_num_batched_tokens)
        data.meta_info["use_dynamic_bsz"] = self.config.rollout.get("log_prob_use_dynamic_bsz", False)
        data.meta_info["temperature"] = data.meta_info.get("temperature", self.config.rollout.temperature)
        top_k = int(data.meta_info.get("analysis_top_k", 5))
        validation_noise_cfg = data.meta_info.get("validation_noise")
        is_validate = data.meta_info.get("validate", False)

        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data)
            with self._apply_validation_noise_to_model_if_needed(self.actor_module, validation_noise_cfg, is_validate):
                output = self.rollout_policy_analyzer.analyze_responses(data=data, top_k=top_k)
            output = self.ulysses_sharding_manager.postprocess_data(output)
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during analyze_generation_policy", logger=logger)
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if config.get("distributional", False):
                hidden_size = critic_module.config.hidden_size
                quantile_embed_dim = getattr(config, "quantile_embed_dim", 64)
                quantile_mode = config.get("quantile_mode","iqn")
                num_value_heads = int(getattr(config, "num_value_heads", 1))
                if quantile_mode == "fixed":
                    critic_module.qr_head = QRValueHead(hidden_size=hidden_size, num_quantiles=config.num_quantiles)
                elif quantile_mode == "c51":
                    critic_module.c51_head = C51ValueHead(hidden_size=hidden_size, num_atoms=config.num_quantiles)
                else:
                    if num_value_heads > 1:
                        critic_module.iqn_head = MultiHeadIQNValueHead(
                            hidden_size=hidden_size,
                            num_heads=num_value_heads,
                            num_cos_embeddings=quantile_embed_dim,
                        )
                    else:
                        critic_module.iqn_head = IQNValueHead(hidden_size=hidden_size, num_cos_embeddings=quantile_embed_dim)
