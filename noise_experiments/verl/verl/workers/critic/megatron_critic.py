...
class MegatronPPOCritic(BasePPOCritic):
    ...
    def compute_values(self, data: DataProto) -> DataProto:
        ...
        is_distributional = getattr(self.config, "distributional", False)
        quantile_mode = getattr(self.config, "quantile_mode", "iqn")
        num_quantiles = getattr(self.config, "num_quantiles", 1)
        quantile_huber_kappa = getattr(self.config, "quantile_huber_kappa", 1.0)
        with torch.no_grad():
        ...
        def loss_func(output, data, meta_info):
            ...
            if is_distributional:
                vf_loss = core_algos.compute_quantile_value_loss(
                    ...
                    tau_mode=quantile_mode,
                    kappa=quantile_huber_kappa,
                )
                vf_clipfrac = torch.tensor(0.0, device=vpreds.device)
                vpred_mean = masked_mean(vpreds.mean(dim=-1), response_mask).detach().item()
            else:
