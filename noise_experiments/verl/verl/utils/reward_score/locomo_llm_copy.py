...
# Lazy-start / idle-stop judge server settings.
_judge_lock = threading.Lock()
_judge_process: Optional[subprocess.Popen] = None
_judge_last_used_ts = 0.0
_judge_idle_thread: Optional[threading.Thread] = None
_judge_idle_stop_evt = threading.Event()
...
def _models_endpoint(base_url: Optional[str]) -> Optional[str]:
    if not base_url:
        return None
    return base_url.rstrip("/") + "/models"


def _judge_healthy(base_url: Optional[str], timeout_s: float = 1.0) -> bool:
...
def _start_idle_monitor_if_needed() -> None:
    global _judge_idle_thread
    if _judge_idle_thread and _judge_idle_thread.is_alive():
        return

    def _monitor():
        while not _judge_idle_stop_evt.is_set():
            timeout_s = int(os.getenv("JUDGE_IDLE_TIMEOUT", "0") or "0")
            if timeout_s <= 0:
                time.sleep(1.0)
                continue
            with _judge_lock:
                proc = _judge_process
                last_ts = _judge_last_used_ts
            if proc and proc.poll() is None and last_ts > 0 and (time.time() - last_ts) >= timeout_s:
                _stop_local_judge_process()
            time.sleep(1.0)

    _judge_idle_stop_evt.clear()
    _judge_idle_thread = threading.Thread(target=_monitor, name="llm-judge-idle-monitor", daemon=True)
    _judge_idle_thread.start()


def _touch_judge_last_used() -> None:
    global _judge_last_used_ts
    with _judge_lock:
        _judge_last_used_ts = time.time()


def _build_judge_launch_cmd() -> list[str]:
    launch_cmd = os.getenv("JUDGE_LAUNCH_CMD")
    if launch_cmd:
        # User can provide full command string.
        return launch_cmd.strip().split()

    model_path = os.getenv("JUDGE_MODEL_PATH")
    if not model_path:
        raise RuntimeError("JUDGE_MODEL_PATH is required when JUDGE_AUTOSTART=1 and JUDGE_LAUNCH_CMD is not set.")
    served_model = os.getenv("JUDGE_SERVED_MODEL", os.getenv("EVALUATOR_MODEL", "judge-14b"))
    port = os.getenv("JUDGE_PORT", "8001")
    tp = os.getenv("JUDGE_TP", "1")
    gpu_util = os.getenv("JUDGE_GPU_MEMORY_UTILIZATION")
    max_model_len = os.getenv("JUDGE_MAX_MODEL_LEN")
    max_num_batched_tokens = os.getenv("JUDGE_MAX_NUM_BATCHED_TOKENS")
...
def _start_local_judge_process_if_needed() -> None:
    global _judge_process
    autostart = os.getenv("JUDGE_AUTOSTART", "0") == "1"
    if not autostart:
        return
    if _judge_healthy(_evaluator_base_url, timeout_s=0.8):
        _touch_judge_last_used()
        _start_idle_monitor_if_needed()
        return
    ...
    with _judge_lock:
        ...
        cmd = _build_judge_launch_cmd()
        env = os.environ.copy()
        judge_cuda = os.getenv("JUDGE_CUDA_VISIBLE_DEVICES")
        if judge_cuda:
            env["CUDA_VISIBLE_DEVICES"] = judge_cuda
        log_path = os.getenv("JUDGE_LOG_PATH")
        if log_path:
            os.makedirs(os.path.dirname(log_path), exist_ok=True)
            log_file = open(log_path, "a", encoding="utf-8")  # noqa: SIM115
            _judge_process = subprocess.Popen(cmd, env=env, stdout=log_file, stderr=subprocess.STDOUT)
        else:
            _judge_process = subprocess.Popen(cmd, env=env)
        print(f"[llm_judge_copy] started judge process pid={_judge_process.pid}", flush=True)

    ready_timeout_s = int(os.getenv("JUDGE_READY_TIMEOUT", "180") or "180")
    start_ts = time.time()
    while time.time() - start_ts < ready_timeout_s:
        if _judge_healthy(_evaluator_base_url, timeout_s=0.8):
            _touch_judge_last_used()
            _start_idle_monitor_if_needed()
            return
        with _judge_lock:
            ...
            if proc and proc.poll() is not None:
                raise RuntimeError(f"judge process exited early with code {proc.returncode}")
        time.sleep(1.0)
    raise RuntimeError(f"judge server not ready within {ready_timeout_s}s")


def _stop_local_judge_process() -> None:
    global _judge_process, _judge_last_used_ts
    with _judge_lock:
        proc = _judge_process
        _judge_process = None
        _judge_last_used_ts = 0.0
    ...
    if proc.poll() is None:
        ...
        try:
            ...
            proc.kill()
    print("[llm_judge_copy] stopped idle judge process", flush=True)


def _cleanup_judge_runtime() -> None:
    _judge_idle_stop_evt.set()
    _stop_local_judge_process()
    _close_client()
    shutdown_event_loop()


atexit.register(_cleanup_judge_runtime)

def release_judge_now() -> None:
    """Force release judge resources so GPU memory can be reclaimed before critic forward."""
    _close_client()
    _stop_local_judge_process()
...
def _close_client() -> None:
    ...
    if loop:
        ...
        try:
            ...
            print(f"[llm_judge_copy] client close failed: {exc}")
        return
...
def get_client() -> "AsyncOpenAI":
    global _client
    _start_local_judge_process_if_needed()
    if _client is None:
        with _client_lock:
            if _client is None:
...
def _ensure_event_loop() -> asyncio.AbstractEventLoop:
    ...
    with _event_loop_lock:
        ...
        thread = threading.Thread(target=_run_loop, name="llm-judge-copy-event-loop", daemon=True)
        thread.start()
...
def _log_retry(retry_state: RetryCallState):
    ...
    message = f"[llm_judge_copy] retry attempt {attempt}"
    if max_attempts:
...
async def evaluate_llm_judge_async(question, gold_answer, generated_answer) -> int:
    try:
        if isinstance(gold_answer, (list, tuple)):
            ...
            for outcome in results:
                if isinstance(outcome, Exception):
                    print(f"[llm_judge_copy] single-gold error: {outcome}", flush=True)
                    continue
                value = int(outcome)
                if value == 1:
                    return 1
        ...
        print(f"[llm_judge_copy] evaluation failed: {exc}", flush=True)
        return 0
